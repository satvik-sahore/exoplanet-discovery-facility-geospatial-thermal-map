...
const projection = d3.geoMercator()
  .scale(150) // Adjust the scale as needed
...
let countryColorScale = d3.scaleSequential(d3.interpolateOrRd)
...
function updateVisualization(selectedDate) {
  ...
  countryColorScale.domain([0, maxCountryCount]);
...
function renderMap() {
  ...
  countries
    .attr("fill", d => d.properties.exoplanetCount > 0 ? countryColorScale(d.properties.exoplanetCount) : "#f0f0f0")
...
// Render Country-Level Thermal Map Legend
function renderCountryLegend(maxCountryCount) {
  // Remove existing country legend if any
  ...
  const legendY = height - 100; // Positioned above the facility legend with gap

  // Create a new legend group for country thermal map
  const legendSvg = svg.append("g")
    .attr("class", "legend-country")
    .attr("transform", `translate(${legendX}, ${legendY})`);

  // Define gradient for country thermal map
  ...
  linearGradientCountry.selectAll("stop")
    .data([
      {offset: "0%", color: countryColorScale(0)},
      {offset: "100%", color: countryColorScale(maxCountryCount)}
    ])
    .enter().append("stop")
